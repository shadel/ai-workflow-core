...
import { loadTaskCompleteConfig } from './task-config-loader.js';
...
export class TaskQueueManager {
  ...
  async completeTask(
    taskId: string,
    options?: { autoActivateNext?: boolean }
  ): Promise<{ completed: Task; nextActive?: Task }> {
    // Validate taskId
    if (!taskId || typeof taskId !== 'string' || taskId.trim().length === 0) {
      throw new Error('Task ID is required and must be a non-empty string');
    }
    ...
    return await this.withLock(async () => {
      ...
      // Determine if should auto-activate
      // Priority: options parameter > config file > default (true)
      // This matches Full Build behavior where CLI flags override config
      let shouldAutoActivate: boolean;

      if (options?.autoActivateNext !== undefined) {
        // Explicit option passed (e.g., from CLI flag)
        // Same as Full Build: CLI flags override config
        shouldAutoActivate = options.autoActivateNext;
      } else {
        // Load from config file
        // Path resolution: Same as Full Build (process.cwd() relative)
        // Full Build resolves config path relative to process.cwd()
        const projectRoot = process.cwd();
        const config = await loadTaskCompleteConfig(projectRoot);
        shouldAutoActivate = config.autoActivateNext;
      }

      // Auto-activate next QUEUED task (by priority) - only if enabled
