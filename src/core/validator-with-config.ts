...
export class ValidatorWithConfig extends Validator {
  ...
  private async loadConfig(): Promise<void> {
    ...
    try {
      ...
      // Using string literal to avoid TypeScript compile-time check
      const fullBuildPath = '@shadel/ai-workflow-full/dist/core/config-manager.js';
      const fullBuildModule = await import(fullBuildPath);
      const { getConfig } = fullBuildModule;
