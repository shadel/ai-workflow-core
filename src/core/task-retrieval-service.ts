    ...
    if (activeQueueTask) {
      ...
      if (await fs.pathExists(this.taskFile)) {
        try {
          ...
          // Also sync if file has old format (goal instead of originalGoal)
          const hasOldFormat = fileData.goal && !fileData.originalGoal;
          if (fileData.taskId !== activeQueueTask.id || hasOldFormat) {
            if (TRACK) {
              console.log('[TRACK TaskManager.getCurrentTask] ⚠️ File taskId MISMATCH or old format! File:', fileData.taskId, 'Queue:', activeQueueTask.id, hasOldFormat ? '(old format detected)' : '', '- Will sync from queue');
            }
            if (DEBUG) {
              console.log('[DEBUG TaskManager.getCurrentTask] File taskId mismatch or old format, syncing from queue:', {
                fileTaskId: fileData.taskId,
                queueTaskId: activeQueueTask.id,
                hasOldFormat
              });
            }
            // File has different task or old format - sync from queue
            ...
            // PERFORMANCE OPTIMIZATION: Pass already-read fileData to avoid re-reading
            // File taskId matches queue - check for manual edit (content changes)
            const manualEditDetected = await this.fileSync.detectManualEdit(activeQueueTask, fileData);
            ...
            if (manualEditDetected) {
              ...
              // PERFORMANCE OPTIMIZATION: Quick consistency check before expensive validation
              // If state and key fields match, skip expensive validation
              const queueState = activeQueueTask.workflow?.currentState || 'UNDERSTANDING';
              const fileState = fileData.workflow?.currentState || 'UNDERSTANDING';
              const statesMatch = queueState === fileState;
              
              // Only do expensive validation if states don't match or we suspect inconsistency
              if (!statesMatch) {
                // States don't match - do full validation
              ...
              // If states match and no manual edit detected, file is in sync - skip expensive validation
            }
          }
        } catch (error) {
