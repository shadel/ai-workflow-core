...
export class TaskFileSync {
  ...
  private async loadExistingFile(): Promise<TaskFileData | null> {
    ...
    try {
      return await fs.readJson(this.taskFile);
    } catch (error) {
   ...
   */
  async detectManualEdit(queueTask: QueueTask): Promise<boolean> {
    const DEBUG = process.env.DEBUG_TASK_FILE_SYNC === 'true';
    ...
    try {
      ...
      const isManualEdit = fileHash !== queueHash && fileData.taskId === queueTask.id;
      ...
      if (DEBUG) {
        console.log('[DEBUG TaskFileSync.detectManualEdit]', {
          ...
          taskIdMatch: fileData.taskId === queueTask.id,
