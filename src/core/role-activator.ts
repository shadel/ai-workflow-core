...
const ROLE_TRIGGERS: Record<string, RoleTrigger> = {
  ...
  devops: {
    roleId: 'devops',
    keywords: [
      'deploy', 'deployment', 'ci/cd', 'pipeline', 'docker', 'dockerfile',
      'kubernetes', 'k8s', 'infrastructure', 'aws', 'azure', 'gcp',
      'terraform', 'ansible', 'monitoring', 'logging',
      'devops', 'container', 'orchestration', 'cloud', 'yml', 'yaml'
    ],
...
export class RoleActivator {
  ...
  private shouldActivateRole(
    ...
    // Only apply priority requirement if there are linkedRequirements
    // If no requirements, priority check doesn't block activation
    if (trigger.minPriority && context.linkedRequirements && context.linkedRequirements.length > 0) {
  ...
  /**
   * Get trigger information for a specific role
   */
  getRoleTriggers(roleId: string): RoleTrigger | undefined {
    return ROLE_TRIGGERS[roleId];
  }

  /**
   * Get all role triggers
   */
  getAllTriggers(): Record<string, RoleTrigger> {
