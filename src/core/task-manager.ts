...
export class TaskManager {
  ...
  async createTask(goal: string, requirements?: string[], force = false): Promise<Task> {
    ...
    if (queueTask.status === 'ACTIVE') {
      ...
      // Build task data with requirements
      const existingData = await fs.pathExists(this.taskFile)
        ? await fs.readJson(this.taskFile).catch(() => ({}))
        : {};
      const taskData = {
        ...
        status: 'in_progress',
        startedAt: queueTask.createdAt,
        workflow: queueTask.workflow || {
          currentState: 'UNDERSTANDING',
          stateEnteredAt: queueTask.createdAt,
          stateHistory: []
        }
      };
      // Use syncFileFromQueue but ensure requirements are included
      await this.syncFileFromQueue(queueTask, requirements && requirements.length > 0 ? ['requirements'] : []);
      // Update requirements: set to provided requirements or empty array (never undefined)
      const fileData = await fs.readJson(this.taskFile);
      fileData.requirements = requirements && requirements.length > 0 ? requirements : [];
      await fs.writeJson(this.taskFile, fileData, { spaces: 2 });
  ...
  async getCurrentTask(): Promise<Task | null> {
        ...
        try {
          ...
          if (fileTaskData.taskId && fileTaskData.status !== 'completed') {
            ...
            if (activeQueueTask && queue.activeTaskId !== fileTaskData.taskId) {
              // DEBUG: Log fixing active task ID
              if (process.env.DEBUG_TASK_MANAGER) {
                console.log('[DEBUG] getCurrentTask() - Fixing activeTaskId, setting to:', fileTaskData.taskId);
              }
              ...
              // DEBUG: Log fixed
              if (process.env.DEBUG_TASK_MANAGER) {
                console.log('[DEBUG] getCurrentTask() - Active task ID fixed');
              }
    ...
    if (activeQueueTask) {
      // Fix 7: Detect manual file edits and auto-sync from queue
      if (await fs.pathExists(this.taskFile)) {
        try {
          const fileTaskData = await fs.readJson(this.taskFile);
          const fileStats = await fs.stat(this.taskFile);
          const fileModified = fileStats.mtime;
          const queueUpdated = new Date(activeQueueTask.workflow?.stateEnteredAt || activeQueueTask.createdAt);
          
          // Check if file was manually edited (file modified after queue update)
          // Allow 2 second tolerance for file system timing differences
          const timeDiff = fileModified.getTime() - queueUpdated.getTime();
          if (timeDiff > 2000 && fileTaskData.taskId === activeQueueTask.id) {
            // File was manually edited, sync from queue
          ...
          } else if (fileTaskData.taskId !== activeQueueTask.id) {
            // Task ID mismatch - sync from queue
            console.warn(`⚠️  Warning: File taskId (${fileTaskData.taskId}) != Queue taskId (${activeQueueTask.id}). Syncing from queue...`);
            await this.syncFileFromQueue(activeQueueTask, ['requirements']);
          }
        } catch (error) {
   ...
   * @param queueTask Task from queue to sync
   * @param preserveFields Fields to preserve from existing file
   */
  private async syncFileFromQueue(
    queueTask: any,
    ...
    // Load existing file data to preserve fields
    const existingData = await fs.pathExists(this.taskFile)
      ? await fs.readJson(this.taskFile).catch(() => ({}))
      : {};
    ...
    try {
      ...
      // Write to temp file
      await fs.writeJson(tempFile, taskData, { spaces: 2 });
      
      // Force flush (with error handling for Windows permission issues)
      try {
        const fd = await fsPromises.open(tempFile, 'r+');
        try {
          await fd.sync();
        } catch (syncError) {
          // Ignore fsync errors on Windows (EPERM can occur)
          if (process.platform !== 'win32' || !(syncError as any).code?.includes('EPERM')) {
            throw syncError;
          }
        } finally {
          await fd.close();
        ...
        // If file operations fail, continue without sync (file is still written)
        console.warn(`⚠️  Warning: Could not sync temp file: ${(error as Error).message}`);
      }
      ...
      try {
        const finalFd = await fsPromises.open(this.taskFile, 'r');
        try {
          await finalFd.sync();
        } catch (syncError) {
          // Ignore fsync errors on Windows
          if (process.platform !== 'win32' || !(syncError as any).code?.includes('EPERM')) {
            throw syncError;
          }
        } finally {
        ...
        // If sync fails, continue (file is still written)
        console.warn(`⚠️  Warning: Could not sync final file: ${(error as Error).message}`);
      }
      
      // Platform-specific delay for Windows
      if (process.platform === 'win32') {
        await new Promise(resolve => setTimeout(resolve, 50));
      }
      
      // Validate sync succeeded
      const verifyData = await fs.readJson(this.taskFile);
      if (verifyData.taskId !== queueTask.id) {
        throw new Error(`Sync validation failed: File taskId (${verifyData.taskId}) != Queue taskId (${queueTask.id})`);
      }
      ...
      if (await fs.pathExists(backupFile)) {
        await fs.copyFile(backupFile, this.taskFile);
      }
  ...
  async updateTaskState(state: WorkflowState): Promise<void> {
    ...
    if (validationData && validationData.workflow) {
      try {
        await this.validateStateHistory(validationData);
      } catch (error) {
    ...
    // Use currentState from file data if available (may be corrupted, but validation should have caught it)
    ...
    // Use file data if available for accurate timing (file has the actual stateEnteredAt)
    const rapidChangeData = fileTaskData && fileTaskData.workflow ? fileTaskData : taskData;
  ...
  private async saveReviewChecklist(checklist: any): Promise<void> {
    ...
    try {
      ...
      // If queue update fails, continue to save in current-task.json
      // This ensures checklist is saved even if queue system has issues
      console.warn('Warning: Failed to save checklist to queue:', (error as Error).message);
    }
    
    // Always update current-task.json for backward compatibility
    // This ensures checklist is saved even if queue system is not available
    ...
    try {
      ...
      // Update task data with checklist
      ...
      // Verify write succeeded (only if checklist was provided)
      if (checklist) {
        const verifyData = await fs.readJson(this.taskFile);
        if (!verifyData.reviewChecklist) {
          throw new Error('Checklist verification failed after write');
        }
      }
      
      savedToFile = true;
    } catch (error) {
      // If file write fails, try one more time with a simpler approach
      console.warn('Warning: Failed to save review checklist to file:', (error as Error).message);
      try {
        await fs.ensureDir(this.contextDir);
        const existingData = await fs.pathExists(this.taskFile) 
          ? await fs.readJson(this.taskFile).catch(() => ({}))
          : {};
        existingData.reviewChecklist = checklist;
        await fs.writeJson(this.taskFile, existingData, { spaces: 2 });
        
        // FIX: Force event loop to process file system flush
        await new Promise(resolve => setImmediate(resolve));
        
        // Verify again (only if checklist was provided)
        if (checklist) {
          const verifyData = await fs.readJson(this.taskFile);
          if (verifyData.reviewChecklist) {
            savedToFile = true;
          } else {
            throw new Error('Checklist verification failed after retry');
          }
        } else {
          savedToFile = true;
        }
      } catch (retryError) {
        console.error('Error: Failed to save review checklist after retry:', (retryError as Error).message);
        throw new Error(`Failed to save checklist: ${(retryError as Error).message}`);
      }
    }
    
    // Log success
    if (savedToFile || savedToQueue) {
      // Success - checklist saved
      if (process.env.DEBUG) {
        console.log('✅ Checklist saved successfully');
      }
    } else {
      throw new Error('Failed to save checklist to both queue and file');
    }
  }
  ...
  async completeTask(): Promise<{ alreadyCompleted: boolean }> {
    ...
    // Update current-task.json for backward compatibility
    if (await fs.pathExists(this.taskFile)) {
      const taskData = await fs.readJson(this.taskFile);
      taskData.status = 'completed';
      taskData.completedAt = result.completed.completedAt;
      
      // FIX: Preserve startedAt if it exists, or set it from task creation time
      if (!taskData.startedAt && result.completed.createdAt) {
        taskData.startedAt = result.completed.createdAt;
      }
      
      // FIX: Preserve all existing fields (requirements, originalGoal, etc.)
      // Don't overwrite existing data, only update status and completedAt
    ...
    if (result.nextActive && result.nextActive.status === 'ACTIVE') {
      ...
      // No next task - clear context files
      await fs.remove(path.join(this.contextDir, 'STATUS.txt'));
      await fs.remove(path.join(this.contextDir, 'NEXT_STEPS.md'));
      await fs.remove(path.join(this.contextDir, 'WARNINGS.md'));
    }
  ...
  async updateTask(
    ...
    // Try to update queue system first
    ...
    // Also update current-task.json for backward compatibility
    if (await fs.pathExists(this.taskFile)) {
      const taskData = await fs.readJson(this.taskFile);
