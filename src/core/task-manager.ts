...
import { StateChecklistService } from './state-checklist-service.js';
...
export class TaskManager implements StateActionHandler, RateLimitingChecker, StateHistoryUpdater, StatePersistenceHandler, StatePrerequisitesValidator {
  ...
  // REFACTORED: Phase 5 - Extract Review Checklist Service (backward compatibility)
  private reviewChecklistService: ReviewChecklistService;
  
  // Phase 1.2: State Checklist Service (supports all states)
  private stateChecklistService: StateChecklistService;
  private checklistRegistry: ChecklistRegistry;
  ...
  constructor(contextDir = '.ai-context') {
    ...
    // REFACTORED: Phase 5 - Initialize Review Checklist Service (backward compatibility)
    ...
    // Phase 1.2: Initialize State Checklist Service (supports all states)
    // Phase 2.3: Inject PatternProvider for pattern-based checklists
    this.checklistRegistry = new ChecklistRegistry();
    const patternProvider = this.contextInjector.getPatternProvider();
    this.stateChecklistService = new StateChecklistService(
      this.queueManager,
      this.fileSync,
      this.taskFile,
      this.checklistRegistry,
      patternProvider
    );
  ...
  async handleStateSpecificActions(state: WorkflowState): Promise<void> {
    ...
    // Phase 3.3: Initialize checklist for all states (including REVIEWING - now uses unified system)
    try {
      await this.stateChecklistService.initializeStateChecklist(state);
    } catch (error) {
      // If checklist initialization fails, log but don't block state transition
      // State is already saved, so we continue (graceful degradation)
      console.warn(`⚠️ Warning: Failed to initialize ${state} checklist: ${(error as Error).message}`);
    }
  }
  ...
  private checkRapidStateChange(taskData: any): void {
    // Skip rate limiting warnings in test mode (tests change states quickly)
    // Unless explicitly enabled for tests that need to test rate limiting behavior
    if ((process.env.NODE_ENV === 'test' || process.env.SKIP_RATE_LIMITING === 'true')
   ...
   * Phase 3.2: Enhanced with checklist blocking for all states
   */
  async validateStatePrerequisites(state: WorkflowState): Promise<void> {
    // Phase 3.2: Block transition if current state checklist is incomplete
    const currentTask = await this.getCurrentTask();
    const currentState = currentTask?.status as WorkflowState | undefined;
    
    // Skip check for UNDERSTANDING state (first state, no checklist needed)
    // Also skip if transitioning to same state (no-op)
    if (currentState && currentState !== 'UNDERSTANDING' && currentState !== state) {
      try {
        await this.stateChecklistService.validateStateChecklistComplete(currentState);
      } catch (error) {
        if (error instanceof StateChecklistIncompleteError) {
          // Re-throw with friendly message
          throw error;
        }
        // For other errors, log warning but don't block (graceful degradation)
        console.warn(`⚠️  Warning: Checklist validation error for ${currentState}:`, (error as Error).message);
      }
    }
    
    // Existing warning logic (keep all existing behavior)
    switch (state) {
      case 'TESTING':
      ...
      case 'READY_TO_COMMIT':
        // Check if REVIEWING checklist is complete using unified system
        // This is already handled by validateStatePrerequisites above (line 670)
        // which checks currentState checklist completion before transition
        // Unified system validates REVIEWING checklist completion automatically
        break;
    }
  }
   ...
   */
  async completeTask(options?: { autoActivateNext?: boolean }): Promise<{ alreadyCompleted: boolean }> {
    return this.completionService.completeTask(options);
  }
