...
export class ContextInjector {
  ...
  /**
   * Get PatternProvider instance (for sharing with other services)
   * Phase 2.3: Allow StateChecklistService to access PatternProvider
   */
   ...
   */
  /**
   * Validate context for STATUS.txt generation
   * @throws Error if context is invalid
   */
  private validateContext(context: ContextInjectionContext): void {
    if (!context?.task) {
      throw new Error('Context must include task information');
    }
    if (!context.task.id || !context.task.goal || !context.task.status) {
      throw new Error('Task must include id, goal, and status');
    }
  }

  /**
   * Generate patterns section with state-aware display
   * Shows top 3 patterns by priority (mandatory first, then recommended)
   */
  private async generatePatternsSection(
    ...
    const totalCount = context.localRules.length;
    const currentState = context.task.status;
    
    // Try to get state-specific patterns from PatternProvider
    try {
      const statePatterns = await this.patternProvider.getPatternsForState(currentState);
      
      // Combine mandatory and recommended, with mandatory first
      const allPatterns = [
        ...statePatterns.mandatory.map(p => ({ ...p, isMandatory: true })),
        ...statePatterns.recommended.map(p => ({ ...p, isMandatory: false }))
      ];
      
      if (allPatterns.length > 0) {
        // Get top 3 patterns (mandatory first, then recommended)
        const topPatterns = allPatterns.slice(0, 3);
        
        const patternsList = topPatterns.map(p => {
          // Determine emoji based on mandatory status and severity
          const emoji = p.isMandatory 
            ? '🔴'  // Mandatory = CRITICAL
            : p.validation.severity === 'error'
            ? '🔴'
            : p.validation.severity === 'warning'
            ? '🟠'
            : '🟡';
          
          // Determine priority label
          const priority = p.isMandatory ? 'CRITICAL' : p.validation.severity.toUpperCase();
          
          // Truncate title if > 50 characters
          const title = p.title.length > 50 
            ? `${p.title.substring(0, 50)}...` 
            : p.title;
          
          return `  ${emoji} ${title} [${priority}] - ID: ${p.id}`;
        }).join('\n');
        
        return `
📚 Active Project Patterns (State: ${currentState}):
  ${totalCount} pattern(s) total
  
  Top priorities:
${patternsList}
  
  See NEXT_STEPS.md for full list
  ⚠️  AI must follow all project patterns!`;
      }
    } catch (error) {
      // PatternProvider not available or error - use fallback
    }
    
    // Simple fallback
    return `
📚 Active Project Patterns:
  ${totalCount} pattern(s) - See NEXT_STEPS.md for details
  ⚠️  AI must follow all project patterns!`;
  }
  ...
  private async generateStatusFile(
    ...
    // Extract validated task properties
    const taskId = context.task.id;
    const taskGoal = context.task.goal;
    const taskState = context.task.status;
    ...
    try {
      ...
      // Filter in memory
      const queuedTasks = allTasks.filter(t => t.status === 'QUEUED').slice(0, 5);
      const completedTasks = allTasks.filter(t => t.status === 'DONE').slice(0, 3);
...
`;
    ...
    // Generate patterns section (async)
    const patternsSection = await this.generatePatternsSection(context);
...
Current Task:
  ID: ${taskId}
  Goal: ${taskGoal}
  State: ${taskState}${activeTaskPriority}
...
⚠️  MANDATORY WORKFLOW ACTIONS:

After reading this file or switching tasks, you MUST:

1. ✅ Run: npx ai-workflow checklist status
   → Review checklist items for current state
   → Mark completed items as you work

2. ✅ Follow checklist items before progressing state
   → Complete required items before state transition
   → Verify optional items for quality
...
Commands:
  ...
  npx ai-workflow checklist status  # Review checklist (MANDATORY)
