...
// WorkflowState is only used as a type (WorkflowState[], type assertions)
// Task is not used in this file, but kept for consistency
...
export class Validator {
   ...
   * @param contextDir - Directory containing .ai-context files (default: '.ai-context')
   * @param configDir - Directory containing config files (default: 'config')
   */
  constructor(contextDir = '.ai-context', configDir = 'config') {
...
${this.getNextStepsForState(currentState)}`
  ...
  async isResultsStale(results: ValidationResults): Promise<boolean> {
    ...
    try {
      const taskFile = path.join(this.contextDir, 'current-task.json');
      // FIX: Ensure directory exists before checking path
      await fs.ensureDir(this.contextDir);
      if (await fs.pathExists(taskFile)) {
        const taskData = await fs.readJson(taskFile);
        if (results.taskId && taskData.taskId !== results.taskId) {
