...
export async function preCommitHook(): Promise<number> {
  ...
  try {
    ...
    // Try to use cached results first (with Cursor verifications)
    let results = await validator.validateAll({ useCachedResults: true });
    
    // Check if results are stale
    const cached = await validator.loadValidationResults();
    if (cached && await validator.isResultsStale(cached)) {
      console.log(chalk.yellow('⚠️  Cached results are stale, running fresh validation...\n'));
      // Re-run validation (but still apply Cursor verifications)
      results = await validator.validateAll({ useCachedResults: false });
    } else if (cached) {
      console.log(chalk.dim('   Using cached results (verified by Cursor)\n'));
    }
