...
export class TaskMigration {
  ...
  async migrate(dryRun: boolean = false): Promise<MigrationResult> {
    try {
      // Ensure directory exists before migration
      await fs.ensureDir(this.contextDir);
      ...
      // Read current task (file should exist if needsMigration returned true)
      ...
      if (!dryRun) {
        // BUG-FIX-011: Don't migrate completed tasks as active tasks
        // Completed tasks should not be created in the queue as active
        if (currentTask.status === 'completed' || currentTask.completedAt) {
          // Task is completed - don't create it in queue
          // Just create tasks.json with empty tasks array
          const emptyQueue = {
            tasks: [],
            activeTaskId: null,
            metadata: {
          ...
          await fs.writeJson(this.tasksFile, emptyQueue, { spaces: 2 });
