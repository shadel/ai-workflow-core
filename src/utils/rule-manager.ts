...
`
  ...
  /**
   * Generate template for a rule (placeholder)
   */
  async generateTemplate(ruleId: string, projectRoot: string): Promise<void> {
    // Placeholder: Would generate rule template file
    const templatePath = path.join(projectRoot, '.ai-context', 'rules', `${ruleId}.md`);
    await fs.ensureDir(path.dirname(templatePath));
    await fs.writeFile(templatePath, `# ${ruleId}\n\nTemplate for ${ruleId}\n`, 'utf-8');
  }

  /**
   * Validate rules format (placeholder)
   */
  async validateRules(projectRoot: string): Promise<{
    valid: number;
    details: any[];
  }> {
    const rules = await this.getRules();
    return {
      valid: rules.length,
      details: rules.map(r => ({ id: r.id, valid: true }))
    };
  }
}
