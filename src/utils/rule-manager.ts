...
export class RuleManager {
   ...
   * v3.1.0: Saves to patterns.json if it exists, otherwise rules.json
   */
  private async saveRules(rules: Rule[]): Promise<void> {
    // Determine which file to use
    const targetFile = await fs.pathExists(this.patternsFile)
      ? this.patternsFile
      : this.rulesFile;
    
    await fs.ensureDir(path.dirname(targetFile));

    // Use appropriate key name based on file
    const keyName = targetFile.includes('patterns') ? 'patterns' : 'rules';
