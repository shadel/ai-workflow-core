...
Examples:
  ...
  $ npx ai-workflow checklist check run-tests --state TESTING

Marks the specified checklist item as complete.
`)
    .action(async (itemId: string, options: { state?: string }) => {
      try {
        ...
        // Mark item complete
        await checklistService.markItemComplete(state, itemId);
        
        console.log(chalk.green(`✅ Checklist item "${itemId}" marked as complete.`));
