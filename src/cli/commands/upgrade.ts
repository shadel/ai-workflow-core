...
async function syncCursorRules(projectRoot: string): Promise<void> {
  ...
  try {
    ...
    const commandsDestPath = path.join(projectRoot, 'docs', 'COMMANDS.md');
    const commandsCreated = await createCommandsMD(projectRoot, commandsDestPath);
    if (commandsCreated) {
      console.log(chalk.green(`   ✅ docs/COMMANDS.md`));
    }
    ...
    console.log(chalk.green(`  ✅ Created: docs/COMMANDS.md`));
    console.log('');
    ...
    console.log(chalk.cyan('  • COMMANDS.md created for command discovery'));
    console.log('');
