...
export const COMMAND_HELPS: CommandHelp[] = [
  ...
  // PATTERNS commands
  {
    ...
    examples: [
      ...
      'npx ai-workflow pattern add "Use async/await" -c "Always use async/await, never .then()"',
      'npx ai-workflow pattern search "validation"',
      'npx ai-workflow pattern check',
      'npx ai-workflow pattern info PATTERN-123',
      'npx ai-workflow pattern export patterns.md'
    ],
    workflow: [
      '1. List patterns: npx ai-workflow pattern list',
      '2. Add pattern: npx ai-workflow pattern add "<title>" -c "<content>"',
      '3. Search: npx ai-workflow pattern search <keyword>',
      '4. Check missing: npx ai-workflow pattern check',
      '5. Export: npx ai-workflow pattern export <file>'
    ],
    tips: [
      'Patterns are learned rules and best practices',
      'Use pattern list to see all patterns',
      'Use pattern check to find missing recommended patterns',
      'Patterns can have validation rules and state requirements'
    ],
    related: ['rule'],
    category: 'patterns',
    priority: 7,
