...
Note:
  ...
  - Use "task list" to see all tasks`)
    .action(async (goal: string, options: { satisfies?: string; priority?: string; estimate?: string; tags?: string; force?: boolean }) => {
      try {
        ...
        let createdTask;
        try {
          createdTask = await taskManager.createTask(goal, requirements, options.force || false);
        } catch (error) {
          handleCliError(error, {
            command: 'task',
            operation: 'create',
            suggestions: [
              'Check if active task exists (use --force to override)',
        ...
        // Only continue if createTask succeeded
  ...
  // task status
  // @requirement REQ-MDC-OPTIMIZATION-002 - Command helpers (--silent, --state-only)
  // @requirement REQ-MDC-OPTIMIZATION-003 - Checklist integration
  task
    .command('status')
    .description('Show current task status')
    .option('--json', 'Output in JSON format')
    .option('--silent', 'Suppress non-JSON output (use with --json)')
    .option('--state-only', 'Return only current state (not full status)')
    .action(async (options: { json?: boolean; silent?: boolean; stateOnly?: boolean }) => {
      try {
        const current = await taskManager.getCurrentTask();
        
        // Handle --state-only option
        ...
        if (options.json) {
          // Get checklist for current state
          // @requirement REQ-MDC-OPTIMIZATION-003 - Checklist integration in task status
          ...
          if (current?.status) {
            try {
              ...
              const checklistService = new StateChecklistService(
                queueManager,
                fileSync,
                path.join('.ai-context', 'current-task.json'),
                registry,
                patternProvider
              );
              
              const checklist = await checklistService.loadStateChecklist(current.status as WorkflowState);
              if (checklist) {
                checklistData = {
              ...
              // Checklist not available, continue without it
            }
          }
          
          // Generate nextActions based on state
          // @requirement REQ-MDC-OPTIMIZATION-002 - Structured output with nextActions
          const nextActions: NextAction[] = [];
          if (current?.status) {
            // Always suggest getting checklist
            nextActions.push({
              type: 'command',
            ...
            // State-specific nextActions
            if (current.status === 'REVIEWING') {
              nextActions.push({
                type: 'command',
                action: 'npx ai-workflow validate --json --silent',
              ...
              nextActions.push({
                type: 'command',
                action: 'npx ai-workflow sync --state REVIEWING',
                reason: 'Progress to REVIEWING after tests pass',
                required: false
              });
            }
          }
          ...
          const output = formatCommandOutput(statusData, nextActions, {
            json: true,
        ...
        // Phase 4.3: Show checklist status
        try {
          const { StateChecklistService } = await import('../../core/state-checklist-service.js');
          const { ChecklistRegistry } = await import('../../core/checklist-registry.js');
          const { TaskQueueManager } = await import('../../core/task-queue.js');
          ...
          if (checklist && checklist.items.length > 0) {
            ...
            const requiredCompleted = checklist.items.filter(i => i.required && i.completed).length;
            
            console.log(chalk.bold('Checklist Status:'));
            console.log(`  Progress: ${chalk.yellow(`${completed}/${total}`)} items completed`);
            if (required > 0) {
              console.log(`  Required: ${requiredCompleted >= required ? chalk.green(`${requiredCompleted}/${required}`) : chalk.red(`${requiredCompleted}/${required}`)} completed`);
            }
            console.log('');
            
            // Phase 4: Enhanced prompt (conditional)
            if (completed < total) {
              // PROMINENT CHECKLIST PROMPT (incomplete)
              console.log(chalk.bold.yellow('╔══════════════════════════════════════════════════════════════════════╗'));
              console.log(chalk.bold.yellow('║ ⚠️  CHECKLIST INCOMPLETE - ACTION REQUIRED                            ║'));
              console.log(chalk.bold.yellow('╚══════════════════════════════════════════════════════════════════════╝'));
              console.log('');
              console.log(chalk.bold('You MUST review and complete checklist items:'));
              console.log(chalk.cyan('  → Run: npx ai-workflow checklist status'));
              console.log(chalk.cyan('  → Review all items'));
              console.log(chalk.cyan('  → Mark completed items'));
              console.log('');
            } else {
              // Subtle prompt for complete checklists
              console.log(chalk.green('✅ All checklist items completed!'));
              console.log(chalk.gray('💡 Use "npx ai-workflow checklist status" to review checklist\n'));
            }
          }
        } catch (error) {
          // If checklist display fails, continue anyway (graceful degradation)
          if (process.env.DEBUG) {
            console.warn(chalk.yellow(`⚠️  Warning: Could not load checklist: ${(error as Error).message}`));
          }
        }
  ...
  // task state
  // @requirement REQ-MDC-OPTIMIZATION-002 - Command helpers for programmatic use
  task
    .command('state')
    .description('Get current workflow state only')
    .option('--json', 'Output in JSON format')
  ...
  task
    ...
    .option('--auto-activate', 'Force auto-activate next task (overrides config)')
    .option('--no-auto-activate', 'Skip auto-activate next task (overrides config)')
    .action(async (options: { autoActivate?: boolean; noAutoActivate?: boolean }) => {
      try {
        ...
        // Parse auto-activate options
        // Commander.js: --no-auto-activate sets autoActivate = false
        // Priority: explicit CLI option > config > default
        let autoActivateNext: boolean | undefined;
        if (options.autoActivate === false) {
          // --no-auto-activate was used
          autoActivateNext = false;
        } else if (options.autoActivate === true) {
          // --auto-activate was used
          autoActivateNext = true;
        }
        // If undefined, use config (handled in TaskQueueManager)

        // BUG-FIX: Explicit error handling for critical operation
        let result;
        try {
          result = await taskManager.completeTask({ autoActivateNext });
        } catch (error) {
          handleCliError(error, {
            command: 'task',
            operation: 'complete',
            suggestions: [
              'Ensure task is at READY_TO_COMMIT state',
        ...
        // Only continue if completeTask succeeded
  ...
  - Use "task list" to see all available tasks`)
    .action(async (taskId: string) => {
      try {
        ...
        let activatedTask;
        try {
          activatedTask = await queueManager.activateTask(taskId);
        } catch (error) {
          handleCliError(error, {
            command: 'task',
            operation: 'switch',
        ...
        // REFACTORED: Use TaskManager.syncFileFromQueue() (now public)
        // This ensures proper sync with TaskFileSync and preserves requirements
        await taskManager.syncFileFromQueue(activatedTask, ['requirements']);
        
        // Phase 3: Show checklist status
        try {
          const { StateChecklistService } = await import('../../core/state-checklist-service.js');
          const { ChecklistRegistry } = await import('../../core/checklist-registry.js');
          const { TaskFileSync } = await import('../../core/task-file-sync.js');
          ...
          const queueManager = new TaskQueueManager();
          const fileSync = new TaskFileSync();
          const registry = new ChecklistRegistry();
          const checklistService = new StateChecklistService(
            queueManager,
            fileSync,
          ...
          const state = activatedTask.workflow?.currentState || 'UNDERSTANDING';
          ...
          if (checklist && checklist.items.length > 0) {
            ...
            // PROMINENT CHECKLIST PROMPT
            console.log(chalk.bold.red('╔══════════════════════════════════════════════════════════════════════╗'));
            console.log(chalk.bold.red('║ ⚠️  MANDATORY ACTION REQUIRED                                         ║'));
            console.log(chalk.bold.red('╚══════════════════════════════════════════════════════════════════════╝'));
            console.log('');
            console.log(chalk.bold('After switching tasks, you MUST:'));
            console.log(chalk.cyan('  1. Run: npx ai-workflow checklist status'));
            console.log(chalk.cyan('  2. Review checklist items for current state'));
            console.log(chalk.cyan('  3. Mark completed items as you work'));
            console.log('');
          }
        } catch (error) {
