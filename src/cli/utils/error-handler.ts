...
export function handleCliError(error: unknown, context: ErrorContext): void {
  ...
  // BUG-FIX: Ensure we always exit on error
  // Use setTimeout to ensure exit happens even if there are pending async operations
  process.exit(1);
  
  // Note: process.exit() terminates execution, so code after it is unreachable
  // TypeScript requires explicit return type, but this function never returns
  // The unreachable throw statement was removed to fix test failures
}
