...
describe('SourceSyncChecker', () => {
  const testDir = '.test-source-sync';
  ...
  describe('detectChanges()', () => {
    it('should detect when source is newer than doc', async () => {
      ...
      // Ensure file is flushed and timestamp is updated
      // CI environments may need more time for file system sync
      ...
      const docUpdated = new Date(beforeWrite - 2000); // 2 seconds before file write
    ...
    it('should return false when doc is newer', async () => {
      ...
      const docUpdated = new Date(Date.now() + 1000); // Future date
      const result = await checker.detectChanges(file, docUpdated);
