...
describe('Bug #9: State History Validation', () => {
  ...
  describe('Detect Corrupted State History', () => {
    ...
    it('should BLOCK forged skip in history (skip DESIGNING)', async () => {
      ...
      try {
        ...
        // Accept either "Current state found in history" or "State mismatch detected"
        expect(
          error.message.includes('Current state found in history') ||
          error.message.includes('State mismatch detected')
        ).toBe(true);
      }
    });
  ...
  describe('Security: Prevent Workflow Bypass', () => {
    ...
    it('should prevent quality gate bypass', async () => {
      // This test performs file operations and state validation - may take longer in parallel execution
    ...
    }, 30000); // 30s timeout for file operations and state validation in parallel execution
  });
  ...
  describe('Edge Cases', () => {
    it('should handle backward movement in history correctly', async () => {
      // This test performs multiple state transitions and file operations - may take longer in parallel execution
      await taskManager.createTask('Test backward tracking', [], true);
      
      // Normal progression
      await taskManager.updateTaskState('DESIGNING');
      await new Promise(resolve => setTimeout(resolve, 100)); // Allow file sync
      await taskManager.updateTaskState('IMPLEMENTING');
      await new Promise(resolve => setTimeout(resolve, 100)); // Allow file sync
    ...
    }, 30000); // 30s timeout for multiple state transitions and file operations in parallel execution
  ...
  describe('Regression Tests', () => {
    ...
    it('should not add performance overhead to valid operations', async () => {
      ...
      // Note: This test includes completeReviewChecklist() which adds significant overhead
      // Use higher multiplier for tests that include checklist completion
      const { getPerformanceThreshold } = await import('./test-helpers.js');
      const baseThreshold = getPerformanceThreshold('full-workflow');
      const threshold = baseThreshold * 3; // 3x multiplier for checklist completion overhead
