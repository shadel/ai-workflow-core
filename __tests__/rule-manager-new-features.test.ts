...
describe('RuleManager - New Features', () => {
  ...
  describe('checkMissingRules()', () => {
    ...
    it('should detect present rules', async () => {
      // Add a rule with a recommended ID by directly manipulating the rules file
      const rules = [{
        id: 'RULE-001',
        title: 'Test Rule',
      ...
      await fs.writeJson(path.join(testDir, '.ai-context', 'rules.json'), {
        rules,
      ...
      expect(result.present).toContain('RULE-001');
    });
  });
  ...
  describe('getRuleInfo()', () => {
    ...
    it('should return rule info with parsed sections', async () => {
      // Add a rule with structured content and known ID
      const rules = [{
        id: 'RULE-001',
        title: 'Test Rule',
