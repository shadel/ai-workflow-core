...
describe('TaskManager', () => {
  ...
  describe('updateTaskState', () => {
    ...
    it('should update task state to READY_TO_COMMIT', async () => {
      ...
      // Complete review checklist before proceeding to READY_TO_COMMIT
  ...
  describe('completeTask', () => {
    ...
    it('should set completedAt timestamp', async () => {
      ...
      // Check if task file exists (may be removed with queue system)
      if (await fs.pathExists(`${testContextDir}/current-task.json`)) {
        const taskData = await fs.readJson(`${testContextDir}/current-task.json`);
        expect(taskData.status).toBe('completed');
        expect(taskData.completedAt).toBeDefined();
      } else {
        // With queue system, check tasks.json
        const tasksFile = `${testContextDir}/tasks.json`;
        if (await fs.pathExists(tasksFile)) {
          const tasksData = await fs.readJson(tasksFile);
          const completedTask = tasksData.tasks.find((t: any) => t.status === 'DONE');
    ...
    it('should handle completing non-existent task', async () => {
      ...
      // Also remove tasks.json if it exists (queue system)
      await fs.remove(`${testContextDir}/tasks.json`);
