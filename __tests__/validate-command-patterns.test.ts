...
describe('Validate Command - Pattern Compliance', () => {
  ...
  describe('Pattern validation reporting', () => {
    ...
    it('should report violations but NOT block commit', async () => {
      // Create a pattern with file_exists validation
      ...
      // But validation should still allow commit (non-blocking)
      // This is tested by the fact that validate command doesn't exit with code 1
      // for pattern violations alone
      expect(violations[0].severity).toBe('error');
      // The key is: violations are reported, but commit is not blocked
      // This is verified by the validate command implementation
    });
