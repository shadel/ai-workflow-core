...
describe('Pattern Commands (v3.1.0)', () => {
  ...
  describe('Pattern operations', () => {
    ...
    test('should add pattern to patterns.json (always, never creates rules.json)', async () => {
      const pattern = await manager.addRule({
        title: 'New Pattern',
        content: 'Pattern content',
        score: 5
      ...
      // Verify saved to patterns.json (v3.1.0+ always uses patterns.json)
      const data = await fs.readJson('.ai-context/patterns.json');
      expect(data.patterns).toHaveLength(1);
      
      // Verify rules.json was NOT created
      expect(await fs.pathExists('.ai-context/rules.json')).toBe(false);
    });
  });
  ...
  describe('Backward compatibility', () => {
    ...
    test('should always save to patterns.json (never creates rules.json)', async () => {
      // Add pattern - should always use patterns.json
      const pattern = await manager.addRule({
        title: 'Test',
        content: 'Content'
      });

      // Should save to patterns.json (v3.1.0+ always uses patterns.json)
      ...
      // Verify it's in patterns.json
      const data = await fs.readJson('.ai-context/patterns.json');
      expect(data.patterns).toHaveLength(1);
      expect(data.patterns[0].title).toBe('Test');
    });
  });
});
