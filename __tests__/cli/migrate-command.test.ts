...
describe('CLI: migrate command', () => {
  ...
  describe('migrate --update-mdc-files', () => {
    it('should copy templates to .cursor/rules/', async () => {
      ...
      // Note: getTemplatesDir() resolves to real package templates, not test templates
      await updateMdcFilesFromTemplates(testDir);
      
      // Then: Files updated from real package templates
      const updatedFile = await fs.readFile(
        path.join(testDir, '.cursor/rules/000-workflow-core.mdc'),
        'utf-8'
      );
      // Verify: File copied from real templates (should contain real template content)
      expect(updatedFile).toBeTruthy();
      expect(updatedFile.length).toBeGreaterThan(0);
      // Verify: All 3 consolidated files are copied
      const files = await fs.readdir(path.join(testDir, '.cursor/rules'));
      const mdcFiles = files.filter((f: string) => f.endsWith('.mdc') && !f.startsWith('.backup'));
      expect(mdcFiles.length).toBeGreaterThanOrEqual(3);
      expect(mdcFiles).toContain('000-workflow-core.mdc');
      expect(mdcFiles).toContain('001-state-behaviors.mdc');
      expect(mdcFiles).toContain('002-quality-gates.mdc');
    });
