...
describe('Task Complete - Improved UX', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle task file with missing status field', async () => {
      ...
      // Remove status field to test edge case
      const taskData = await fs.readJson(taskFile);
      delete taskData.status;
      await fs.writeJson(taskFile, taskData);
      
      // Without status field and without completedAt, should not be considered completed
      const current = await manager.getCurrentTask();
      expect(current).not.toBeNull(); // Returns task because no status and no completedAt
    });
  ...
  describe('Multiple Task Lifecycle', () => {
    it('should handle multiple task completions in sequence', async () => {
      // This test performs multiple task completions in sequence
      // Timeout set to 20000ms via it() parameter
    ...
    }, getTestTimeout() * 2); // Enhanced timeout for Windows/parallel execution
  });
