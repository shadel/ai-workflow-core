...
describe('TaskQueueManager Structure', () => {
  ...
  describe('Lock Options Configuration', () => {
    it('should configure lock options with retry settings', () => {
      ...
      // Updated for 75% maxWorkers requirement - increased retries and timeouts
      expect(lockOptions.retries.retries).toBe(30);
      expect(lockOptions.retries.minTimeout).toBe(200);
      expect(lockOptions.retries.maxTimeout).toBe(3000);
    });
  });
