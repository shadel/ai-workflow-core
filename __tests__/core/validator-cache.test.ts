...
describe('Validator - Cursor Verification Cache', () => {
  ...
  describe('saveValidationResults', () => {
    it('should save validation results to context', async () => {
      await fs.ensureDir(testContextDir);
      await fs.writeJson(path.join(testContextDir, 'current-task.json'), {
        taskId: 'test-123',
        originalGoal: 'Test',
      ...
      const resultsFile = path.join(testContextDir, 'validation-results.json');
    ...
    it('should include commit hash if in git repo', async () => {
      ...
      const saved = await fs.readJson(path.join(testContextDir, 'validation-results.json'));
    ...
    it('should preserve existing Cursor verifications when saving', async () => {
      ...
      await fs.writeJson(path.join(testContextDir, 'validation-results.json'), {
  ...
  describe('loadValidationResults', () => {
    ...
    it('should load validation results from context', async () => {
      ...
      await fs.writeJson(path.join(testContextDir, 'validation-results.json'), testResults);
    ...
    it('should return null if file is corrupted', async () => {
      await fs.ensureDir(testContextDir);
      await fs.writeFile(path.join(testContextDir, 'validation-results.json'), 'invalid json');
  ...
  describe('validateAll with caching', () => {
    ...
    it('should run fresh validation if cached results are stale', async () => {
      ...
      await fs.writeJson(path.join(testContextDir, 'validation-results.json'), oldResults);
