...
describe('ContextInjector', () => {
  ...
  describe('timestamps', () => {
    ...
    /**
     * @requirement STATUS.txt - Checklist Instruction
     * @isolation Uses unique test directory via getUniqueAIContextDir()
     */
    it('should include checklist instruction in STATUS.txt', async () => {
      const context: ContextInjectionContext = {
        task: mockTask,
        warnings: [],
        blockers: []
      ...
      expect(content).toContain('MANDATORY WORKFLOW ACTIONS');
      expect(content).toContain('npx ai-workflow checklist status');
    });

    /**
     * @requirement STATUS.txt - Checklist Command in List
     * @isolation Uses unique test directory via getUniqueAIContextDir()
     */
    it('should include checklist command in commands list', async () => {
      const context: ContextInjectionContext = {
        task: mockTask,
        warnings: [],
        blockers: []
